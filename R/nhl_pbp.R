...
#' @description update_nhl_db() updates or creates a database with `powerplay`
...
#' are available for `powerplay`.
#'
#' The argument `force_rebuild` is of hybrid type. It can rebuild the play
#' by play data table either for the whole powerplay era (with `force_rebuild = TRUE`)
...
update_nhl_db <- function(dbdir = ".",
                          dbname = "powerplay_db",
                          tblname = "powerplay_nhl_pbp",
                          force_rebuild = FALSE,
                          db_connection = NULL) {
  old <- options(list(stringsAsFactors = FALSE, scipen = 999))
  on.exit(options(old))
  
  rule_header("Update powerplay Play-by-Play Database")
  ...
  # if (length(missing) > 0) {
  #   new_pbp <- build_powerplay_pbp(missing, rules = FALSE)
  #   
  #   if (nrow(new_pbp) == 0) {
...
# this is a helper function to build powerplay database from Scratch
build_nhl_db <- function(tblname = "powerplay_nhl_pbp", db_conn, rebuild = FALSE, show_message = TRUE) {
